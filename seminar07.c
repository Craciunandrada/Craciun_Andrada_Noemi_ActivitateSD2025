...
void filtreazaHeap(Heap heap, int pozitieNod) {
	...
	if (fiuStanga < heap.nrElemente && heap.masini[pozitieNod].id < heap.masini[fiuStanga].id)
	{
		pozMax = fiuStanga;
	}
	if (fiuDreapta < heap.nrElemente && heap.masini[pozitieNod].id < heap.masini[fiuDreapta].id)
	{
		pozMax = fiuDreapta;
	}
...
Heap citireHeapDeMasiniDinFisier(const char* numeFisier) {
	...
	Heap heap=initializareHeap(10); //initialiam heap ul
	while (!feof(f))
	{   
	...
	//cu filtrarea pornim de jos, apelam incepand cu ultimul parinte
...
int main() {
	Heap h;
	h = citireHeapDeMasiniDinFisier("masini_arbore.txt");
	Masina masina;
	masina=extrageMasina(&h); //daca return void, apelam extrageMasina()
